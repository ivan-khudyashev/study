...
public class ComparatorTest
{
	...
	static void test1()
	{
		...
		al.sort((Person a, Person b) -> a.compareTo(b));
		System.out.println(al);
		...
		System.out.println("Sort by length of LastName, then Age");
		/*
		al.sort(
			(
				(Person p1, Person p2) 
					-> p1.getLastname().length() - p2.getLastname().length()
			).thenComparing(
				(Person p) -> p.getAge()
			)
		);
